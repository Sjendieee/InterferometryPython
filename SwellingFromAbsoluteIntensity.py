...
def main():
    ...
    source = "C:\\Users\ReuvekampSW\\Documents\\InterferometryPython\\export\\PROC_20231004114924"

    range1 = 3300#2320       #start x left for plotting
    range2 = 3900  # len(swellingProfile)

    ###wedge trial swelling ratios
    knownPixelPosition = 3897 - range1 - 1 #pixellocation at which the bursh height is known at various times
    dryBrushThickness = 190       #160                 # dry brush thickness (measured w/ e.g. ellipsometry)
    idxArrToUse = [0]         #id of csv files to use [0, 50, 95, 206, 395]?
    knownHeightArr = [190]   #Known brush swelling at pixellocation in nm for certain timesteps   #in nm
    ...
    EVALUATERIGHTTOLEFT = False         #evaluate from left to right, or the other way around    (required for correct conversion of intensity to height profile)
                                        #(Usually left to right (False), meaning left is CL and drier region on the right)
